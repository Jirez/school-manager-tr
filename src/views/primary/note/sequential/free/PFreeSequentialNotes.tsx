...
import {
  ...
  usePFreeSequentialNoteQuery,
  useSubCompetencesByClassQuery,
  useSubPeriodsQuery,
} from '@/gql/graphql'
import PFreeSequentialNoteAdd from './PFreeSequentialNoteAdd'

const PFreeSequentialNotes = () => {
  const [clazz, setClazz] = useState<{ [key: string]: any }>()
  const [subCompetence, setSubCompetence] = useState<{
    [key: string]: any
  } | null>()
  const [subPeriod, setSubPeriod] = useState<any[]>([])
  ...
  const {
    ...
    variables: {
      ...
      subPeriodIds: subPeriod ? subPeriod.map((i: any) => Number(i.id)) : [],
      subCompetenceId: subCompetence ? Number(subCompetence.id) : -1,
      schoolId: enterpriseId,
    },
    skip:
      !clazz ||
      (!subPeriod && (subPeriod as any)?.length === 0) ||
      !subCompetence,
    fetchPolicy: 'no-cache',
  })
          ...
          <div className="w-full">
            {loadingNote ? (
              ...
              <div className="card" style={{ marginTop: 20 }}>
                <PFreeSequentialNoteAdd
                  sequentialNotes={dataNote?.sequentialNotes}
                  classId={clazz.id}
                  subPeriodId={subPeriod.map((i) => Number(i.id))}
...
export default PFreeSequentialNotes
