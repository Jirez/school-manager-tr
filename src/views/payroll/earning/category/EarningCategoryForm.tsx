...
import { earningCategoryValidation } from './earning.category.validation'
...
interface FormProps extends BaseFormProps {
  category?: EarningCategoryType
  modal?: NiceModalHandler
}

const EarningCategoryForm: React.FC<FormProps> = ({
  category,
  modal,
  action,
  ...props
  ...
  const {
    ...
    //@ts-ignore
    resolver: yupResolver(earningCategoryValidation),
  })
  ...
  const onSubmit = (
    ...
    return handleSubmit(async (values) => {
      ...
      action({
        ...
        .then(async ({ data }) => {
          reset()
          toast.success(`Catégorie ${data.earningCategory.name} ajoutée`, {
            ...TOAST_OPTIONS,
          })
          if (close) {
            modal?.hide()
          ...
          if (props.popover) {
            messageService.sendMessage('earningCategory', data.earningCategory)
            props.onModalClose?.()
          }
        })
        .catch((error) => {
  ...
  return (
    <Form onSubmit={onSubmit}>
      <div className="grid grid-cols-1 md:grid-cols-1 gap-x-1 gap-y-1">
        <FormSection
          title={
            t('label-earningCategoryInfo') || 'Informations de la catégorie'
          }
          description={
            t('label-earningCategoryInfoDesc') ||
        ...
        >
          <div className="space-y-3">
            ...
            <ToggleOption
              ...
              description={t('label-activeDesc') || 'Catégorie active'}
...
export default EarningCategoryForm
