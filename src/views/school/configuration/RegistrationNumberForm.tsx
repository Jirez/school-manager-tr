...
import { EncodeRegistrationNumberDocument } from '@/gql/graphql'
import {
  FormContainer,
  Section,
  SectionHeader,
...
const RegistrationNumberForm: FC<Props> = ({
  registrationNumber,
  action,
  onCancel,
  ...props
  ...
  const encode = async (json: any) => {
    const { data } = await client.query({
      query: EncodeRegistrationNumberDocument,
      variables: { json },
  ...
  const {
    ...
    formState: { isDirty },
    getValues,
  } = useForm<RegistrationNumberType>({
    defaultValues: {
